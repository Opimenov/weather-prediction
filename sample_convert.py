...
for i in range(0,10):
	df1 = pd.read_csv('./sample_data/%s__0.csv' % (meteo_vars[i]), header=None)
	df2 = pd.read_csv('./sample_data/%s__9.csv' % (meteo_vars[i]), header=None)
	print('read in %s variables\n' % (meteo_vars[i]))
	df2 = df2.tail(9)
	df2.index = range(11314, 11323)
	df1 = df1.append(df2)
	np.save(file=('./preproc_data2/%s.npy' % (meteo_vars[i])), arr=df1.as_matrix())
